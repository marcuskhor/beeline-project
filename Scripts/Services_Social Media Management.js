...
// ==================== (最终波浪版) Platform Icons ==================== //
function initPlatformPop() {
        ...
        tl.add(() => {
            items.forEach((item, i) => {
                ...
                gsap.to(item, {
                    ...
                    duration: 1, // 单程时间
                    ease: "sine.inOut",
                    yoyo: true,    // 只有当前这个图标自己往返
                    repeat: -1,    // 无限循环
                    
                    // 【核心】用 delay 制造波浪，而不是用 stagger
                    // 这样它们就会永远保持 1->2->3->4 的顺序追赶
                    delay: i * 0.2 
                });
            });
        }, "-=0.2"); // 在 Pop 还没完全结束时就开始准备起飞
    }
}
